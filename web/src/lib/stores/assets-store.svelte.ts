...
import { getAssetInfo, getLiteTimeBucket, getTimeBucket, getTimeBuckets, TimeBucketSize, type AssetResponseDto, type LiteTimeBucketAssetResponseDto } from '@immich/sdk';
...
class IntersectingAsset {
  ...
  asset: LiteTimeBucketAssetResponseDto | undefined = $state();
  id: string = $derived.by(() => this.asset!.id);

  constructor(group: AssetDateGroup, asset: LiteTimeBucketAssetResponseDto) {
    this.#group = group;
    this.asset = asset;
  }
}
type AssetOperation = (asset: LiteTimeBucketAssetResponseDto) => { remove: boolean };

type MoveAsset = { asset: LiteTimeBucketAssetResponseDto; year: number; month: number };
...
export class AssetBucket {
  ...
  getAssets() {
    ...
    return this.dateGroups.reduce(
      (accumulator: LiteTimeBucketAssetResponseDto[], g: AssetDateGroup) => accumulator.concat(g.getAssets()),
      [],
    );
  }
  ...
  addAssets(assets: LiteTimeBucketAssetResponseDto[]) {
    const lookupCache: {
      [dayOfMonth: number]: AssetDateGroup;
    } = {};
    const unprocessedAssets: LiteTimeBucketAssetResponseDto[] = [];
...
interface AddAsset {
  type: 'add';
  values: LiteTimeBucketAssetResponseDto[];
}
...
export class AssetStore {
  ...
  connect() {
    const toLiteAsset = (asset: AssetResponseDto) => {
      const { height, width } = getAssetRatio(asset);
      return {
        ...asset,
        height,
        width
    ...
    this.#unsubscribers.push(
      websocketEvents.on('on_upload_success', (asset) => this.#addPendingChanges({ type: 'add', values: [toLiteAsset(asset)] })),
      websocketEvents.on('on_asset_trash', (ids) => this.#addPendingChanges({ type: 'trash', values: ids })),
      websocketEvents.on('on_asset_update', (asset) => this.#addPendingChanges({ type: 'update', values: [toLiteAsset(asset)] })),
  ...
  #getPendingChangeBatches() {
    const batch: {
      add: LiteTimeBucketAssetResponseDto[];
      update: LiteTimeBucketAssetResponseDto[];
      remove: string[];
    } = {
      add: [],
      update: [],
  ...
  async loadBucket(bucketDate: string, options?: { cancelable: boolean }): Promise<void> {
    ...
    const result = await bucket.loader?.execute(async (signal: AbortSignal) => {
      const { liteAssets } = await getLiteTimeBucket(
        {
          ...this.#options,
      ...
      if (liteAssets) {
        if (this.#options.timelineAlbumId) {
          const { liteAssets: liteAlbumAssets } = await getLiteTimeBucket(
            {
          ...
          if (liteAlbumAssets) {
            for (const asset of liteAlbumAssets) {
              this.albumAssets.add(asset.id);
            }
          }
        }

        const unprocessed = bucket.addAssets(liteAssets);
  ...
  addAssets(assets: LiteTimeBucketAssetResponseDto[]) {
    const assetsToUpdate: LiteTimeBucketAssetResponseDto[] = [];
  ...
  #addAssetsToBuckets(assets: LiteTimeBucketAssetResponseDto[]) {
    if (assets.length === 0) {
      return;
    }
  ...
  async #getBucketInfoForAsset(asset: LiteTimeBucketAssetResponseDto, options?: { cancelable: boolean }) {
  ...
  #runAssetOp(ids: Set<string>, op: AssetOperation) {
    ...
    let idsToProcess = new Set(ids);
    const idsProcessed = new Set<string>();
    const combinedMoveAssets: { asset: LiteTimeBucketAssetResponseDto; year: number; month: number }[][] = [];
    for (const bucket of this.buckets) {
      if (idsToProcess.size > 0) {
  ...
  updateAssets(assets: LiteTimeBucketAssetResponseDto[]) {
    const lookup = new Map<string, LiteTimeBucketAssetResponseDto>(assets.map((asset) => [asset.id, asset]));
  ...
  getFirstAsset(): LiteTimeBucketAssetResponseDto | undefined {
    return this.buckets[0]?.getFirstAsset();
  }

  async getPreviousAsset(asset: LiteTimeBucketAssetResponseDto): Promise<LiteTimeBucketAssetResponseDto | undefined> {
  ...
  async getNextAsset(asset: LiteTimeBucketAssetResponseDto): Promise<LiteTimeBucketAssetResponseDto | undefined> {
    let bucket = await this.#getBucketInfoForAsset(asset);
    if (!bucket) {
      return;
    }
  ...
  isExcluded(asset: LiteTimeBucketAssetResponseDto | AssetResponseDto) {
    return (
