...
import type { AssetResponseDto, LiteTimeBucketAssetResponseDto } from '@immich/sdk';
...
export function getJustifiedLayoutFromAssets(
  assets: LiteTimeBucketAssetResponseDto[],
...
export function justifiedLayout(assets: LiteTimeBucketAssetResponseDto[], options: CommonLayoutOptions) {
  const adapter = {
  ...
  const result = createJustifiedLayout(assets, adapter,);
